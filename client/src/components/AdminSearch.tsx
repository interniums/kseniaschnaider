...
export default function AdminSearch() {
  ...
  const filter = () => {
    if (!loading) {
      ...
      const newCollections = collections.filter((item) => item.name.toLowerCase().includes(input.toLowerCase()))
      const newCategories = categories.filter((item) => item.name.toLowerCase().includes(input.toLowerCase()))
            ...
            <>
              <div className="relative flex items-center" onFocus={() => setOpen(true)} ref={dropDownRef}>
                ...
                <input
                  ...
                  onChange={(e) => {
                    ...
                    setOpen(true)
                    adminData.setInput(e.target.value)
                  }}
                  placeholder="Search..."
                />
              </div>
              {open && (
                <div
                  ...
                  className={
                    'w-full grid gap-4 py-2 px-4 border border-t-0 max-h-80 h-min overflow-y-auto rounded-md bg-white transition-all ease-in-out z-10'
                  }
                >
                  {!displayCollections?.length == 0 ? (
                    <div>
                      <div className="mb-4">
                        <Link to="/admin-page/collection" onClick={() => setOpen(false)}>
                          <p className="text-xl mb-2 hover:underline cursor-pointer">Collections</p>
                        </Link>
                        <hr />
                      </div>
                  ...
                  {!displayCategories?.length == 0 ? (
                    <div>
                      <div className="mb-4">
                        <Link to="/admin-page/category" onClick={() => setOpen(false)}>
                          <p className="text-xl mb-2 hover:underline cursor-pointer">Categories</p>
                        </Link>
                        <hr />
                      </div>
                      <div className="grid gap-2">
                  ...
                  {!displayItems?.length == 0 ? (
                    <div>
                      <div className="mb-4">
                        <Link to="/admin-page/item" onClick={() => setOpen(false)}>
                          <p className="text-xl mb-2 hover:underline cursor-pointer">Items</p>
                        </Link>
                        <hr />
                      </div>
                      <div className="grid gap-2">
